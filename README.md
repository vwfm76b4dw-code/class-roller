...
双击 `ClassRoller-4.0.0-setup.exe`，按向导装完即可。
...
解压 `class-roller-4.0.0-win64.zip`，双击 `class-roller\class-roller.exe`。
...
- **背景效果**：设置 →「外观」可选
  - *不透明*：最清晰，兼容性最好（默认）
  - *半透明*：整窗轻微透明，所有系统可用
  - *液态玻璃*：系统级模糊背景，Win11 效果最佳，Win10 用传统亚克力；
    系统不支持时自动退回半透明，不会显示成一片空白

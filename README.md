...
双击 `ClassRoller-3.1.0-setup.exe`，按向导装完即可。
...
解压 `class-roller-3.1.0-win64.zip`，双击 `class-roller\class-roller.exe`。
...
- **统一设计系统**：所有配色、字号、间距、圆角都来自 `theme.py`，
  有测试保证界面代码不写死这些数值
- **应用图标**：蓝底骰子，与界面强调色一致
- 结果卡是视觉主角：中间大字显示姓名，抽中时有一次约 140ms 的淡入

## 功能

- **一键抽奖**：点一下直接出结果
- **公平抽取（默认开）**：随机源用操作系统级加密熵（CSPRNG），序列不可预测；
  且一轮之内不重复点名——所有人被抽过一遍才重新开始，不会冷落任何学生。
  课堂实测"连续抽到同一个人"的尴尬不再发生。可在设置里关闭（关闭后为
  纯均匀随机，允许重复）。
